...
const ChatWindow = () => {
  ...
  if (!currentUser) {
    return (
      <div className="flex-1 flex items-center justify-center glass-card rounded-r-3xl">
        <div className="text-center space-y-4">
          <div className="w-16 h-16 mx-auto bg-gradient-to-r from-[#074C77]/20 to-[#0a6ba8]/20 rounded-full flex items-center justify-center">
            <span className="text-2xl">💬</span>
          </div>
          <p className="text-gray-300">Please log in to view messages</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 flex flex-col h-full">
      {/* Chat Header */}
      <div className="p-6 border-b border-gray-700/50 flex items-center justify-between glass-card-light rounded-tr-3xl">
        <div className="flex items-center space-x-4">
          <div className="relative w-12 h-12">
            <img
              src={getProfilePicture(selectedUser)}
              alt={selectedUser.name}
              className="rounded-full w-full h-full object-cover border-2 border-[#074C77]/50"
            />
            <span className="absolute bottom-0 right-0 w-3 h-3 bg-gradient-to-r from-[#407023] to-[#5a9e32] rounded-full border-2 border-gray-900 animate-pulse"></span>
          </div>
          <div>
            <h3 className="font-semibold text-white text-lg">{selectedUser.name}</h3>
            <p className="text-sm text-[#5a9e32] font-medium">Online</p>
          </div>
        </div>
        {isSubscribed && (
          <div className="flex space-x-3">
            <button
              ...
              className="p-3 rounded-2xl bg-gradient-to-r from-[#074C77]/20 to-[#0a6ba8]/20 hover:from-[#074C77]/40 hover:to-[#0a6ba8]/40 transition-all duration-300 hover:scale-110 hover:shadow-lg hover:shadow-[#074C77]/20"
            >
              <FaPhone className="w-5 h-5 text-[#074C77]" />
            </button>
            <button
              onClick={() => startCall(true)}
              className="p-3 rounded-2xl bg-gradient-to-r from-[#407023]/20 to-[#5a9e32]/20 hover:from-[#407023]/40 hover:to-[#5a9e32]/40 transition-all duration-300 hover:scale-110 hover:shadow-lg hover:shadow-[#407023]/20"
            >
              <FaVideo className="w-5 h-5 text-[#407023]" />
            </button>
          </div>
        )}
      </div>
      ...
      <div
        ...
        className="flex-1 overflow-y-auto p-6 space-y-4 bg-gradient-to-b from-gray-900/50 to-gray-900/80 backdrop-blur-xl"
        style={{ 
          backgroundImage: `linear-gradient(135deg, rgba(7, 76, 119, 0.03) 0%, rgba(64, 112, 35, 0.03) 100%)`,
        }}
      >
        {conversation.map((message, index) => (
          ...
          >
            <div
              className={`max-w-[70%] rounded-2xl p-4 shadow-2xl backdrop-blur-sm transition-all duration-300 ${
                message.senderId === currentUser._id
                  ? "bg-gradient-to-r from-[#074C77] to-[#0a6ba8] text-white shadow-[#074C77]/30 hover:shadow-[#074C77]/50 hover:scale-[1.02]" // Sender's message
                  : "bg-gray-800/90 text-white border border-gray-700/50 shadow-gray-900/50 hover:bg-gray-800/70 hover:border-gray-600/50" // Recipient's message
              }`}
            >
              {message.type === "call" ? (
