...
const Sidebar = () => {
  ...
  if (!currentUser) {
    return (
      <div className="w-1/4 h-[80vh] glass-card rounded-l-3xl border-r border-gray-700/50 flex items-center justify-center">
  ...
  return (
    <div className="w-1/4 h-[80vh] glass-card rounded-l-3xl border-r border-gray-700/50">
      <div className="p-6 border-b border-gray-700/50">
        <h3 className="text-2xl font-bold gradient-text mb-6">Chats</h3>
        ...
        <div className="flex items-center">
          <input
            ...
            className="glass-input flex-1 p-3 rounded-2xl text-white placeholder-gray-400 focus:ring-2 focus:ring-[#074C77]/50"
            value={searchTerm}
            onChange={handleSearchChange}
          />
          <button
            className="ml-3 text-[#074C77] hover:text-[#0a6ba8] transition-all duration-300 hover:scale-110"
      ...
      <div className="overflow-y-auto h-[calc(80vh-120px)]">
        {status === "loading" || isSearching ? (
          <div className="flex justify-center items-center h-32">
            <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#074C77] shadow-lg shadow-[#074C77]/30"></div>
          </div>
        ) : status === "failed" ? (
          <div className="text-center text-red-400 mt-4 p-4">
            <p>{error || "Failed to load chats"}</p>
            <button
              onClick={fetchChats}
              className="mt-2 text-[#074C77] hover:text-[#0a6ba8] transition-colors"
            >
              Try again
            </button>
          </div>
          ...
          sortedItems.map((item) => {
            ...
            return (
              <div
                ...
                className={`flex items-center p-4 mx-2 rounded-2xl hover:bg-gray-800/80 transition-all duration-300 cursor-pointer border-l-4 ${
                  selectedUser?._id === otherUser._id
                    ? "bg-gradient-to-r from-[#074C77]/20 to-[#0a6ba8]/10 border-l-[#074C77] shadow-lg shadow-[#074C77]/20"
                    : "border-l-transparent hover:border-l-[#074C77]/50 hover:shadow-md hover:shadow-[#074C77]/10"
                }`}
              >
                ...
                <div className="ml-4 flex-1">
                  ...
                  {item.isNewChat ? (
                    <p className="text-sm text-[#074C77] font-medium">
