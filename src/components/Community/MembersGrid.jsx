...
const MemberCard = ({ member }) => {
  ...
  return (
    <Link
      ...
      className="bg-gray-800/80 backdrop-blur-sm border border-gray-700/50 rounded-lg p-3 flex items-center space-x-4 hover:shadow-lg hover:shadow-[#074C77]/20 hover:border-[#074C77]/50 transition-all duration-200 hover:scale-105"
    >
      <img
      ...
      <div className="flex-1">
        ...
        {member.speaks && member.speaks.length > 0 && (
          <div className="flex items-center space-x-2 mt-2 text-sm text-gray-400">
            <div className="flex gap-2 items-center">
              <span className="font-medium text-[#074C77]">SPEAKS</span>
              {member.speaks.map((lang, index) => (
                <span key={index} className="ml-1 text-gray-300">
                  {getCountryName(lang)}
                </span>
              ))}
            </div>
        ...
        {member.learns && member.learns.length > 0 && (
          <div className="flex items-center space-x-2 mt-2 text-sm text-gray-400">
            <div className="flex gap-2 items-center">
              <span className="font-medium text-[#5a9e32]">LEARNS</span>
...
const SearchBar = ({ onSearch }) => {
  ...
  return (
    <div className="flex items-center gap-4 max-w-md">
      <div className="flex items-center bg-gray-800/60 backdrop-blur-sm border border-gray-700/50 rounded-full px-4 py-2 flex-grow">
        ...
        <input
          ...
          className="bg-transparent outline-none text-white placeholder-gray-400 flex-grow focus:ring-1 focus:ring-[#074C77] rounded"
        />
      </div>
      <button className="p-2 bg-gray-800/60 backdrop-blur-sm border border-gray-700/50 rounded-full hover:bg-gray-700/60 hover:border-[#074C77]/50 transition-all duration-200">
...
const MembersGrid = () => {
      ...
      <div className="flex gap-5 py-7">
        {tabs.map((tab) => (
          <button
            ...
            className={`flex items-center hover:bg-gray-700/80 transition-all hover:text-white px-4 py-2 rounded-full border ${
              activeTab === tab.name
                ? "bg-gradient-to-r from-[#074C77] to-[#0a6ba8] text-white border-[#074C77]/50 shadow-lg shadow-[#074C77]/30"
                : "bg-gray-800/60 border-gray-700/50 text-gray-300 hover:border-[#074C77]/30"
            }`}
          >
  ...
  let content;
  if (loading) {
    content = <p className="text-gray-300 text-center">Loading...</p>;
  } else if (error) {
    content = <p className="text-red-400 text-center">{error}</p>;
  ...
  return (
    <div className="p-4 min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
