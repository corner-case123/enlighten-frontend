...
export const fetchLoggedInUser = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      // Get user data from localStorage first as a fallback
      const localUser = localStorage.getItem('user') ? JSON.parse(localStorage.getItem('user')) : null;
      
      console.log('Fetching user data with headers:', getHeaders());
      ...
      // If API call fails but we have local user data, use that as a fallback
      const localUser = localStorage.getItem('user') ? JSON.parse(localStorage.getItem('user')) : null;
      if (localUser) {
...
export const logout = createAsyncThunk(
  ...
  async (_, { dispatch, rejectWithValue }) => {
    try {
      ...
      // Remove the token from cookies with proper settings
